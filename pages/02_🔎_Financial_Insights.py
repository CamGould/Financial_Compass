...
Path = "/users/cg/Documents/Personal/Projects/Capstone_Project/Supplemental/the_canadian_investor_ep10.mp3"
podcast1 = open(Path, 'rb')
audio_podcast_1 = podcast1.read()

Path_2 = "/users/cg/Documents/Personal/Projects/Capstone_Project/Supplemental/Moolala_episode.mp3"
podcast1 = open(Path_2, 'rb')
audio_podcast_2 = podcast1.read()
...
tab1, tab2 = st.sidebar.tabs(["Canadian Investors Podcast", "Money Made Simple"])
with tab1:
    with st.expander("The Canadian Investor Podcast", True):
        st.markdown("<h3 style='text-align: center'><em>Top 10 Acquisitions & Why Simon is Selling the Stock</h3>", unsafe_allow_html=True)
        image_podcast1 = Image.open("/users/cg/Documents/Personal/Projects/Capstone_Project/Supplemental/TCIP-photo.jpg")
        st.audio(audio_podcast_1)
        st.image(image_podcast1)
        st.caption("In this release of the *Canadian Investor Podcast*, we discuss the following topics")
        st.caption("- Diversification vs Diworsification")
        st.caption("- Investing in a Recession")
        st.caption("- Top 10 Acquisitions of All Time")
        st.caption("- Why Simon is selling a position")
        st.caption("Tickers of stock discussed: GOOG, ACN, EQIX, ASML, PINS")

with tab2:
    with st.expander("Money Made Simple with Bruce Sellery", True):
        st.markdown("<h3 style='text-align: center'><em>Applying Lessons from Trekking to Investing</h3>", unsafe_allow_html=True)
        image_podcast2 = Image.open("/users/cg/Documents/Personal/Projects/Capstone_Project/Supplemental/moolala_image.jpg")
        st.audio(audio_podcast_2)
        st.image(image_podcast2)
        st.caption("We talk to Salman Ahmed, Chief Investment Officer at Steadyhand Investment Funds, who has trekked all around the world and is going to give us some lessons he learned while trekking and how it applies to investing.")
